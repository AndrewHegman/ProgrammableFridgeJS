...
module.exports = (useHardware) => {
	class LCDController {
		constructor() {
			...
			this.screenId = 3; // Attempting to connect screen
			this.text = null;
			this.oldText = null;
			...
			this.socket.on("buttonPressed", (data) => {
				if (data.hasOwnProperty("button")) {
					if (data.button === 1) {
						this.screenId = this.screenId === 0 ? 1 : 0;
						this.setScreen();
					}
				}
			});
		...
		setScreen(text = null) {
			...
			if (!areScreensEqual(this.text, this.oldText)) {
				// Make sure that LCD is ready to be written to (will always be false if useHardware === false)
				if (this.lcdReady) {
